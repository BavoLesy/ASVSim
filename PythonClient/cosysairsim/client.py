...
# -----------------------------------  Vessel APIs ---------------------------------------------
class VesselClient(VehicleClient, object):
    def __init__(self, ip="", port=41451, timeout_value=3600):
        super(VesselClient, self).__init__(ip, port, timeout_value)

    def setVesselControls(
        self, vehicle_name="", controls: Optional[VesselControls] = None
    ) -> None:
        if not controls:
            controls = VesselControls(0, 0)
        self.client.call("setVesselControls", vehicle_name, controls)
    
    def setDisturbanceControls(
        self, vehicle_name="", controls: Optional[DisturbanceControls] = None
    ) -> None:
        if not controls:
    ...
    def getVesselState(self, vehicle_name=""):
        ...
        return VesselState.from_msgpack(state_raw)
    
    def simAddObstacle(self, pose, speed = 1000.0, obstacle_name = ""):
        """
        Create Obstacle at runtime

        Args:
            pose (Pose): Initial pose of the Obstacle
            speed (float): Movement speed of the obstacle, default 1000.0
            obstacle_name (str): Name of the Obstacle being created, default empty which uses the default blueprint for the
             obstacle type

        Returns:
            bool: Whether Obstacle was created
        """
        return self.client.call('simAddObstacle', pose, speed, obstacle_name)
    
    def activateGeneration(self,landscape = False):
        """
        Create a PCGVolume, PCGWorldActor, generationManager and landscape, these are needed for terrain generation

        Args:
            landscape (bool): Wether to generate a landscape, default false as its possible a landscape already exists

        Returns:
            bool: Whether generation is activated
        """
    ...
    def generatePortTerrain(self, type = "", seed = -464588337, length = 10, mina = -45.0, maxa = 45.0, mind = 3000.0, maxd = 6000.0):
        ...
        Args:
            type (str): Type of terrain being generated, default empty which generates a default cube tarrain
            seed (int): Seed used in the generation
            length (int): length of the generation
            mina (float): minimum angle between points in the generation (degrees)
    ...
    def getGoal(self, initial_location, distance = 12):
        """
        Gets the (relative) locations for a goal to be reached in meters, unreachable goals will return a goal at 0,0

        Args:
            initial_location (vector2r): the absolute starting location in meters
            distance (int): how far in the terrain is the goal, default 12

        Returns:
            list[vector2r]: the goal plus left and right border locations
        """
        return self.client.call('getGoal', distance, initial_location)
    
    def getLocation(self,initial_location):
        """
        Gets the absolute and relative location of the agent in meters

        Args:
            initial_location (vector2r): the absolute starting location in meters

        Returns:
            list[vector2r]: the absolute location and relative location to the initial location
        """
        return self.client.call('getRelLocation',initial_location)