...
class VesselState(MsgpackMixin):
    kinematics_estimated = KinematicsState()
    timestamp = np.uint64(0)


class VesselControls(MsgpackMixin):
    """ 
    #Initialize
    rudder_thrust = 0.0
    rudder_angle = 0.0
    def __init__(self, thrust=0.0, angle=0.0):
        self.rudder_thrust = thrust
        self.rudder_angle = angle

    def set_thrust(self, thrust_val):
        self.rudder_thrust = thrust_val

    def set_angle(self, angle_val):
        self.rudder_angle = angle_val
  
    """
    
    MAX_THRUSTER_COUNT: int
    thruster_forces: List[float]
    thruster_angles: List[float]

    def __init__(self, thrust: Optional[List[float] | float] = None, angle: Optional[List[float] | float] = None):
        self.MAX_THRUSTER_COUNT = 10
        self.thruster_forces = [0.0] * self.MAX_THRUSTER_COUNT
        self.thruster_angles = [0.0] * self.MAX_THRUSTER_COUNT

        if thrust is not None:
            if isinstance(thrust, (int, float)):
                thrust = [thrust] * self.MAX_THRUSTER_COUNT
            for i, thrust_val in enumerate(thrust):
                if i < self.MAX_THRUSTER_COUNT:
                    self.set_thrust(thrust_val, i)
        if angle is not None:
            if isinstance(angle, (int, float)):
                angle = [angle] * self.MAX_THRUSTER_COUNT
            for i, angle_val in enumerate(angle):
                if i < self.MAX_THRUSTER_COUNT:
                    self.set_angle(angle_val, i)

    def set_thrust(self, thrust_val, indx=0):
        if indx < 0 or indx >= self.MAX_THRUSTER_COUNT:
            raise IndexError("Rudder index out of (compile-time) range. " + \
                "If you need more than 10 thrusters, modify 'MAX_THRUSTER_COUNT' in types.py")
        self.thruster_forces[indx] = thrust_val

    def set_angle(self, angle_val, indx=0):
        if indx < 0 or indx >= self.MAX_THRUSTER_COUNT:
            raise IndexError("Rudder index out of (compile-time) range. " + \
                "If you need more than 10 thrusters, modify 'MAX_THRUSTER_COUNT' in types.py")
        self.thruster_angles[indx] = angle_val
    


class DisturbanceControls(MsgpackMixin):
    wind_force = 0.0
    wind_angle = 0.0
    # wave_force = 0.0
    # wave_angle = 0.0
    ...
    def __init__ (self, wind_force = 0.0, wind_angle = 0.0, current_velocity = 0.0, current_angle = 0.0):
        self.wind_force = wind_force
        self.wind_angle = wind_angle
        # self.wave_force = wave_force
        # self.wave_angle = wave_angle
        self.current_velocity = current_velocity
        self.current_angle = current_angle
    
    def set_wind(self, wind_force_val, wind_angle_val):
        self.wind_force = wind_force_val
        self.wind_angle = wind_angle_val
    
    # def set_wave(self, wave_force_val, wave_angle_val):
    #     self.wave_force = wave_force_val
    #     self.wave_angle = wave_angle_val
