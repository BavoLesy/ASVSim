...
bool ASimModeBase::spawnObstacle(const msr::airlib::Pose& pose, float speed, const std::string& obstacle_name)
{
    //set variables
    float min;
    float max;
    ...
    if ((pose.position.x() == 0 && pose.position.y() == 0 && pose.position.z() == 0)) {
        min = -10000.0;
        max = 10000.0;
        location = FVector(FMath::RandRange(min, max), FMath::RandRange(min, max), 190.0);
    }
    else {
        location = FVector(pose.position.x(), pose.position.y(), pose.position.z());
    }
    //set rotation
    if ((pose.orientation.x() == 1.0 && pose.orientation.y() == 1.0 && pose.orientation.z() == 1.0 && pose.orientation.w() == 1.0)) {
        min = -90.0;
        max = 90.0;
        FRotator lookrotation = UKismetMathLibrary::FindLookAtRotation(location, FVector(4100.0, -200.0, 190.0));
        FRotator randrotation = FRotator(0, FMath::RandRange(min, max), 0);
        rotation = lookrotation + randrotation;
    }
    else {
        FQuat quat = FQuat(pose.orientation.x(), pose.orientation.y(), pose.orientation.z(), pose.orientation.w());
        rotation = quat.Rotator();
    }

    std::string npc_path;
    if (obstacle_name == "" || obstacle_name == "boat") {
        npc_path = "Class'/AirSim/Blueprints/BP_NPCSpawn.BP_NPCSpawn_C'";
    }
    if (obstacle_name == "buoy") {
        npc_path = "Class'/AirSim/Blueprints/BP_BuoySpawn.BP_BuoySpawn_C'";
    }
    else {
        npc_path = "Class'/AirSim/Blueprints/BP_NPCSpawn.BP_NPCSpawn_C'";
    }

    //spawn NPC actor while setting variables
    auto BP_NPC = UAirBlueprintLib::LoadClass(npc_path);
;
    FTransform SpawnTransform(FQuat(rotation), location);

    AActor* spawned_npc = GetWorld()->SpawnActorDeferred<AActor>(BP_NPC, SpawnTransform);

    FProperty* prop = spawned_npc->GetClass()->FindPropertyByName(FName("CustomVelocity"));
    if (prop) {
        void* PropertyValue = prop->ContainerPtrToValuePtr<void>(spawned_npc);
		static_cast<FBoolProperty*>(prop)->SetPropertyValue(PropertyValue, true);
		UE_LOG(LogTemp, Warning, TEXT("New CustomVelocity: %d"), *(bool*)PropertyValue);
    }
	FProperty* propS = spawned_npc->GetClass()->FindPropertyByName(FName("Speed"));
    if (propS) {
        FString PropType = propS->GetCPPType();
        void* PropertyValueS = propS->ContainerPtrToValuePtr<void>(spawned_npc);
        static_cast<FDoubleProperty*>(propS)->SetPropertyValue(PropertyValueS, speed);
		UE_LOG(LogTemp, Warning, TEXT("New Speed: %f"), *(double*)PropertyValueS);
    }

    UGameplayStatics::FinishSpawningActor(spawned_npc, SpawnTransform);

    spawned_actors_.Add(spawned_npc);
...
bool ASimModeBase::activateGeneration(bool landscape)
{
    ...
    if (PCGVolume)
    {
        // Set scale and variables
        PCGVolume->SetActorScale3D(Scale);

        UPCGComponent* PCGComponent = PCGVolume->FindComponentByClass<UPCGComponent>();
        if (PCGComponent)
        {
    ...
    if (landscape)
    {
        ...
        HeightData.Init(32768, (MaxX - MinX + 1) * (MaxY - MinY + 1));

        TMap<FGuid, TArray<uint16>> HeightDataMap;
        FGuid LayerGuid = FGuid();
        HeightDataMap.Add(LayerGuid, HeightData);
...
bool ASimModeBase::generatePortTerrain(const std::string& type, int seed, int length, float mina, float maxa, float mind, float maxd)
{
    ...
    Params.type = FString(UTF8_TO_TCHAR(type.c_str()));
    Params.mina = mina;
    Params.maxa = maxa;
    Params.mind = mind;
    Params.maxd = maxd;
...
std::vector<FVector2D> ASimModeBase::getGoal(int distance, FVector2D initial_location)
{
    //initialize parameters
	std::vector<FVector2D> port_locations;
    port_locations = { FVector2D(0, 0), FVector2D(0, 0), FVector2D(0, 0) };
    FVector2D port_location = FVector2D(0, 0);
    ...
    UFunction* Function = GenerationManager->FindFunction(FName("getGoal"));
    if (Function) {
        GenerationManager->ProcessEvent(Function, &Params);
		UE_LOG(LogTemp, Warning, TEXT("getGoal: %f, %f"), Params.location.X, Params.location.Y);

        port_location = FVector2D(Params.location.X, Params.location.Y);
		port_left = FVector2D(Params.left.X, Params.left.Y);
		port_right = FVector2D(Params.right.X, Params.right.Y);
		
    }
    else {
        return port_locations;
    }
	if (initial_location == FVector2D(0, 0)) {
		//if initial location is not set, use the character location
        FVector MyCharacter = GetWorld()->GetFirstPlayerController()->GetPawn()->GetActorLocation();
		initial_location = FVector2D(MyCharacter.X /100, MyCharacter.Y /100);
	}

    //calculate relative location in 
    port_location = FVector2D(port_location.X / 100, port_location.Y / 100);
	port_location = FVector2D(port_location.X - initial_location.X, port_location.Y - initial_location.Y);
	
    port_left = FVector2D(port_left.X / 100, port_left.Y / 100);
	port_left = FVector2D(port_left.X - initial_location.X, port_left.Y - initial_location.Y);
	
    port_right = FVector2D(port_right.X / 100, port_right.Y / 100);
	port_right = FVector2D(port_right.X - initial_location.X, port_right.Y - initial_location.Y);

    port_locations[0] = port_location;
    port_locations[1] = port_left;
    port_locations[2] = port_right;

    return port_locations;
}
std::vector<FVector2D> ASimModeBase::getRelLocation(FVector2D initial_location)
{
    std::vector<FVector2D> locations;
	locations = { FVector2D(0, 0), FVector2D(0, 0) };

	//calculate true location in meters
    FVector MyCharacter = GetWorld()->GetFirstPlayerController()->GetPawn()->GetActorLocation();
    
	FVector2D true_location = FVector2D(MyCharacter.X, MyCharacter.Y);
	true_location = FVector2D(true_location.X / 100, true_location.Y / 100);

	//calculate relative location in meters
	FVector2D rel_location = FVector2D(true_location.X - initial_location.X, true_location.Y - initial_location.Y);

	locations[0] = true_location;
	locations[1] = rel_location;
	return locations;
}
